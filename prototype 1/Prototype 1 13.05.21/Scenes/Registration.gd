...
#files in userData saved in format of username,cash,shellMax(20),HPMax(10),hpKitsize(2),ammokitSize(10),normal_sensativity(1),sound(1=true, 0 =false)
#,highscore1,highscore2,highscore3,highscore4,highscore5,type of hud 0 = only text 1 = progress bar 2 = both,how to rotate 0 = left and right arrow 1 = a d keys, how to fire 0 = f 1 =space 2 = r \n
...
func addLogin(user, passw): #adds stuff to the txt files about user
	var file = File.new()
	file.open("res://userData.txt", file.READ)
	var content = file.get_as_text()
	file.close()
	...
	var add = content+"\n"+user +",0,20,10,2,10,1,1,0,0,0,0,0,2,1,1"
